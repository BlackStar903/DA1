...
public class DaoGiamGia implements InterfaceGiamGia {
    ...
    String INSERY_SQL = "insert  dbo.GiamGia values(?,?,?,?,1)";
    String UPDATE_SQL = "update GiamGia set Tensukien=? where Id_GiamGia=?";
    String DELETE_SQL = "UPDATE GiamGia SET trangthai = 0 WHERE Id_GiamGia = ?;\n"
            + "DELETE FROM Giamgiachitiet WHERE Id_GiamGia = ?;";
    ...
    public void delete1(String id, String id1) {
        JDBCHeper.update(DELETE_SQL, id, id1);
    }
    ...
    public void updateDateFinsh(GiamGia e) {
        String sql = "UPDATE GiamGia SET NGAYKT = ?, trangthai = 1 WHERE Id_GiamGia = ?;\n"
                + "DELETE FROM Giamgiachitiet WHERE Id_GiamGia = ?;";
        JDBCHeper.update(sql, e.getNgayKT(),e.getId_GiamGia(), e.getId_GiamGia());
    }
